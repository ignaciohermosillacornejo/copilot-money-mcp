...
| #701 | A docblock in the same gate documented the wrong declaration after a later PR inserted a pair into the gap — the second time for that block, and three more instances across `src/` found by the sweep written to catch it | `doc-reality-drift` | #724 |
