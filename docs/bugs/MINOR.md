...
| #741 | `auto-merge.yml` read the approving identity off `github.repository_owner` — an account that spells a person's login only while the repo is personal, and an organisation login (which reviews nothing) afterwards. Latent and fail-closed, so it would have cost a maintainer an afternoon rather than a merge. The gate written for it found two live siblings: `npm-publish.yml` declared `TRUSTED_PUBLISHERS` with the comment "only these users can publish" and read it nowhere, while three gates carried their own copy of the literal; `auto-release.yml` repeated the login across three job gates with nothing to check them against | `proxy-for-authority` | #752 |
