...
| #643 | `auto-merge.yml` was `pull_request_review`-only; GitHub parks that run at `action_required` for a first-time fork contributor and refuses to release it, so the workflow never fired and never reported failing | `silent-failure-masking` | #723 |
| #692 | Ten of twelve workflow jobs declared no `timeout-minutes`, inheriting GitHub's 360-minute default — ~360x the slowest observed job, and one of them holds `id-token: write` | `config-blind-default` | #723 |
