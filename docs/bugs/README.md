...
| `vacuous-assertion` | A test executes a guard but would still pass if the guard were deleted. Coverage cannot detect this; only mutation can. | partial — `bun run check:mutation-guards` (`scripts/mutation-guards.ts`) applies each registered guard's exact disabling edit and requires its named test file to pass unmutated and fail mutated, so a registered invariant cannot be vacuous. Scoped by construction to the invariants on that ledger: a vacuous assertion elsewhere in the suite is still invisible, and adding the row is a human decision |
| `silent-failure-masking` | An error-handling construct — `continue-on-error`, catch-and-return-null, an allow-list drop — turns a real failure into a green signal, so it surfaces late or never. | partial (mutation-verified) — `check:tracked-files` (`scripts/check-tracked-files.ts`, `tests/scripts/check-tracked-files.test.ts`) closes one mechanism repo-wide: the `.gitignore` allow-list drop that makes `git add` a silent no-op for a file the repo's own tooling reaches (#729). Every other mechanism in the definition — `continue-on-error`, catch-and-return-null, a filter that swallows its rejects — still has nothing static |
| `doc-reality-drift` | Documentation asserts a verifiable property the code no longer has. Invisible to every behavioural gate, because nothing reads Markdown. | `check-privacy-endpoints`, `check:tool-counts` (per-property only); `tests/docs/jsdoc-stranding.test.ts` for the in-code case — a docblock separated from the declaration it describes (#701) |
| `collection-time-assertion` | A test file asserts while the runner is still discovering tests — at module scope, in a `describe` body, or through a helper it calls from one. The failure is reported as an unnamed load error carrying no test name, and every `test()` the throw skipped past is never registered, so the run's test count silently shrinks. | `tests/no-collection-time-assertions.test.ts` (mutation-verified): a TypeScript-AST sweep of every `tests/**/*.ts` — helpers and setup modules included, since a module-scope `expect()` there runs during the collection of every test that imports it — for an `expect()` evaluated at collection time, directly or via a locally declared asserting helper named in a collection-time scope (except where naming cannot be calling — a re-export or an object-literal slot). Per-file syntax only, so an asserting helper imported from another module is out of reach |
...
| `deferred-cleanup-never-runs` | Releasing a resource is deferred to a timer or callback that only fires if the process outlives it — in a process that routinely exits first. The resource is acquired eagerly and released never, while every same-process test still observes correct bookkeeping. | partial — still nothing static, because the class is a deployment property (does this process outlive its own timers?) that no source scan can evaluate. Two deployments are now asserted by running them: `tests/core/leveldb-reader-temp-cleanup.test.ts` (a child process killed normally and by signal) and, since [#642](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/642), `tests/core/temp-db-suite-teardown.test.ts` (a real `bun test` child with a private `$TMPDIR`, plus a teardown-disabled control run) — both mutation-verified |
| `stranded-artifact` | An artifact created as a step toward a durable operation — a temp copy, a `.tmp` awaiting rename — outlives a failure that lands between its creation and the operation completing, because the failure path handles the error but not the artifact. Sibling of `deferred-cleanup-never-runs`: same litter, reached through a failure branch rather than a timer that never fires. | partial — write-then-rename sweep (`tests/core/write-then-rename-sweep.test.ts`): the idiom is structurally confined to `writeFileAtomic`, whose failure-path cleanup is unit-tested through an ops seam. The temp-directory shape (#631) remains instance-only — no static check confines `mkdtemp`-then-populate |
...
| `ambient-state-dependency` | A test reaches live state outside the repository — another application's data directory, the network, the wall clock — usually through a production default it declined to override, so its result is a property of the machine rather than of the code. Nothing distinguishes it from a hermetic test while the machine is quiet, and when the ambient state misbehaves the failure names the code under test. | partial (mutation-verified) — `tests/no-ambient-database-discovery.test.ts`: no `tests/**` file may construct `CopilotDatabase` without an explicit path, which is the only route from the suite to the user's live Copilot container; the five falsy spellings that reach discovery anyway are flagged with it, each pinned by its own specimen line so no member can be dropped from the set with the suite green, and the "only route" premise — one call site, not exported — is asserted rather than inherited from the day it was written (#756). One resource, not the class: no scan can decide whether an arbitrary default reaches the machine |
