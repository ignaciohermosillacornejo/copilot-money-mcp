...
- **Rules** (12 mutations + 15 queries/fields probed): mutations `createRule`, `editRule`, `deleteRule`, `applyRule`, `createTransactionRule`, `editTransactionRule`, `deleteTransactionRule`, `createCategoryRule`, `editCategoryRule`, `deleteCategoryRule`, `setTransactionRule`, `runRules`. **No user-facing rules API exists on GraphQL** — Copilot's categorization rules (the `rule` field on `EditRecurringInput`) are scoped to recurring detection only. **The query side was swept separately on 2026-09-18 and is also absent** (the 2026-04-22 sweep covered mutations only): root queries `rules`, `nameRules`, `partialNameRules`, `categoryRules`, `transactionRules`, `userRules`, `categorizationRules`, `merchantRules`; fields `Category.{partialNameRules,nameRules,rules,rule,matchers}` and `User.{rules,nameRules}`; plus ~40-stem "Did you mean" root fishing (`rule`, `merchant`, `override`, `custom`, `pattern`, `mapping`, `alias`, `learned`, `assign`, …) surfacing nothing rule-adjacent. `Transaction.categoryIdSource` does **not** exist on GraphQL either, so the cache's category provenance has no live-mode equivalent — control probe passed (`suggestedCategoryIds` returned "must not have a selection", confirming the method was sound). The Copilot **web app has no rules UI at all** (Settings is only General / Account / Subscription / Banks & institutions / AI apps / About), so a Chrome capture has nothing to capture on this surface. Cache side is equally empty — see "Categorization rules" under `users/{user_id}/categories` in [`../firestore-collections.md`](../firestore-collections.md).
