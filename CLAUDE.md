...
- **PR rituals (ratchet + evidence classes)**: every PR fills the "External assumptions" section of `.github/PULL_REQUEST_TEMPLATE.md` — each new assumption about Copilot's API/data declares an evidence class (probe transcript / live round-trip / `unverified` + ledger entry in `src/conformance/ledger.ts`); "None" is an explicit answer, not a default. Bug-fix PRs additionally fill the bug-response template in CONTRIBUTING.md ("Bug Response Ritual"): root cause → bug class → class-level detector → siblings checked → ledger updated.
