...
- **How often:** once per credential, not once per request. The first call in a session that needs authentication performs the read described above; every later call refreshes the credential it already holds, against Google's endpoint only. That cached credential is discarded — and the browser read repeated — only when the endpoint says the credential itself is finished (it was revoked, or you logged out). A rate limit, a blocked API identity, or an outage leaves it in place and is simply retried, so a transient failure does not trigger a fresh scan of your browser profiles.
