...
- **Which browsers:** ten are searched — Chrome, Arc, Microsoft Edge, Brave, Vivaldi, Chromium, Opera, Opera GX, Safari, and Firefox (the list lives in `BROWSER_CONFIGS` in `src/core/auth/browser-token.ts`). Chromium-family browsers are searched across every `Default` / `Profile N` directory, not only the default profile; Opera and Opera GX are additionally searched at their user-data root, where older builds keep a single profile.
- **Which files — Chromium family:** the `.ldb` and `.log` files under `IndexedDB/https_app.copilot.money_0.indexeddb.leveldb` — Copilot's own origin — and, **always alongside it, not only when that lookup fails**, `Local Storage/leveldb`. Both directories are read for every profile of every installed Chromium browser; preferring the Copilot-scoped one is only an ordering preference applied to the resulting candidate list — but it is a **global** one: every candidate found under Copilot's own origin is tried before any candidate from a browser-wide store, across all profiles and all browsers.
- **Which files — Safari and Firefox:** neither uses the file-type filter above. **Safari** is the broadest case: two website-data roots (`~/Library/Containers/com.apple.Safari/Data/Library/WebKit/WebsiteData/Default` and `~/Library/Safari/Databases`) are walked recursively — directories up to four levels below each root are entered — and *every* file under 10 MB is scanned regardless of extension. **Firefox** is the narrowest: only profile storage origins whose directory name contains `copilot` are visited, but every file in each such origin's `idb/` directory is scanned.
- **The Local Storage read and the Safari walk are not scoped to Copilot's origin.** A browser's `Local Storage/leveldb` is a single database holding *every* site's local storage, and Safari's website-data roots hold every site's stored data. The server scans those bytes for the Firebase refresh-token pattern and keeps each match as a candidate; nothing else in those files is parsed, retained, or transmitted.
- **A consequence of that:** a refresh token belonging to a different Firebase-backed site you are logged into can be picked up as a candidate. **At most ten candidates** (`MAX_EXCHANGE_CANDIDATES` in `src/core/auth/firebase-auth.ts`) are tried against Google's token-exchange endpoint (see [Network Access](#network-access)) and discarded whenever the endpoint refuses them — because they belong to another Firebase project, or because the bytes were never a usable token at all — so up to ten such tokens may reach Google's endpoint before one is accepted or the attempt gives up. That is a ceiling on the worst case, not the usual case: because the budget is spent Copilot-scoped-first, a logged-in user's session is normally accepted on the first exchange and **no** other site's token is sent anywhere. The ten are a single shared budget, deliberately not one budget per source, so that ordering can only ever reduce how many browser-wide tokens leave your machine. None is ever sent to Copilot Money's API, logged, or written to disk; all candidates are held in memory only.
- **How often:** once per credential, not once per request. The first call in a session that needs authentication performs the read described above. After that, the access token it obtained is held in memory and reused with **no network request at all** until roughly a minute before it expires (they last an hour); only then is Google's endpoint contacted again, to refresh it — never your browser profiles. That cached credential is discarded — and the browser read repeated — only when the endpoint says the credential itself is finished (it was revoked, or you logged out). A rate limit, a blocked API identity, or an outage leaves it in place and is simply retried, so a transient failure does not trigger a fresh scan of your browser profiles.
