...
- **Which files — Chromium family:** the `.ldb` and `.log` files under `IndexedDB/https_app.copilot.money_0.indexeddb.leveldb` — Copilot's own origin — and, as a fallback, `Local Storage/leveldb`.
- **Which files — Safari and Firefox:** neither uses the file-type filter above. **Safari** is the broadest case: two website-data roots (`~/Library/Containers/com.apple.Safari/Data/Library/WebKit/WebsiteData/Default` and `~/Library/Safari/Databases`) are walked recursively to a depth of four, and *every* file under 10 MB is scanned regardless of extension. **Firefox** is the narrowest: only profile storage origins whose directory name contains `copilot` are visited, but every file in each such origin's `idb/` directory is scanned.
- **The Local Storage fallback and the Safari walk are not scoped to Copilot's origin.** A browser's `Local Storage/leveldb` is a single database holding *every* site's local storage, and Safari's website-data roots hold every site's stored data. The server scans those bytes for the Firebase refresh-token pattern and keeps each match as a candidate; nothing else in those files is parsed, retained, or transmitted.
