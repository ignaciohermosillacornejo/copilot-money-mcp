/**
 * Smoke test: get_tags_live against the real Copilot GraphQL endpoint.
 *
 * Run: `bun run scripts/smoke/tags.ts`
 *
 ...
 *   3. refresh_cache --scope tags invalidates and triggers refetch
 *
...
async function main(): Promise<void> {
  ...
  const tools = new LiveTagsTools(live);
  const refresh = new RefreshCacheTool(live);

  // 1. Cold
  const t0 = Date.now();
  const cold = await tools.getTags({});
  ...
  if (cold.count === 0) throw new Error('expected at least one tag');

  // 2. Warm
  const t1 = Date.now();
  const warm = await tools.getTags({});
  ...
  // 3. Refresh + recold
  await refresh.refresh({ scope: 'tags' });
  log('refreshed', { scope: 'tags' });
  const t2 = Date.now();
  const recold = await tools.getTags({});
