...
- **Three cache fields Copilot added after the coverage triage closed at zero: `amazon`, `user_changed_type`, `creation_timestamp`** ([#718](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/718), closing [#611](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/611)). The decoder's unread-field warn triage closed at zero across 28 collections in [#317](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/317); `bun run smoke:cache` against a real cache found it was zero no longer, with 27 distinct `(collection, field)` pairs warning. Three of them are real data, and all three are now decoded and requestable by name — **all opt-in, none added to a `DEFAULT_*_FIELDS` preset**, because this release is about shrinking responses and none of them is read on every call.
  ...
  - **`amazon`** on transaction rows is the valuable one. Copilot's Amazon integration writes the receipt onto the transaction itself: `{order_id, items: [{id, name, link, price, quantity}], other: {giftWrapping, rewards, savings, shipping, tax}}`. That is the data `skills/amazon-sync/SKILL.md` currently obtains from a manually exported Amazon CSV — the skill is deliberately **not** rewritten here, but the input it needs is now one `fields: ["default", "amazon"]` away **in cache mode**. Note the caveat that rewrite will have to solve: that skill writes, `--write` implies `--live-reads`, and the live row has no `amazon` under any spelling (it is built from `ENRICHED_FIELD_MAPPERS`, which maps only what GraphQL returns), so a live caller gets a `_field_warning` rather than a receipt. `get_transactions_live`' description now says so. `get_transactions`' description names the sub-shape rather than only the field, because nothing about a transaction suggests Copilot holds the line items for it and an undiscoverable field is one nobody requests. Present on 26 of 1040 non-empty transaction documents in the probed cache.
  - **`user_changed_type`** (boolean, cache mode only, same reason) marks a transaction whose type a human overrode. Absent on every row nobody re-typed, so read it as "was this overridden?", where absent means no — worth checking before any automation second-guesses a type.
