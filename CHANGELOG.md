...
- **`get_transactions` supports field selection.** Each transaction document carries ~35-40 fields (internal IDs, Plaid metadata, intelligence-suggestion arrays, misc. flags), most of which go unused for typical spending-analysis queries — pulling months of history at full width wastes an MCP client's context and can push a single call over response-size limits. Pass `fields: [...]` to name exactly the fields you want, or `compact: true` for a curated 7-field preset (`transaction_id`, `date`, `name`, `amount`, `category_name`, `account_id`, `pending`). Default behavior (neither option) was unchanged at that point; #604 below then flipped the default to terse rows and removed `compact` before any of this shipped. Fixes [#592](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/592).
