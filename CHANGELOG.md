...
- **`get_accounts` now says what `name` actually is** ([#665](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/issues/665)). Since [#660](https://github.com/ignaciohermosillacornejo/copilot-money-mcp/pull/660) the field carries your Copilot **nickname** when you have set one, and the provider's label otherwise — but nothing said so, including the v3 rewrite of this very description. A client had no way to know the value is user-editable and unsafe as a stable key. The description now states the contract and points at `account_id`.
